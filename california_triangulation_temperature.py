...
app.layout = html.Div([
    ...
    html.H1("Triangulated California Temperature Map For Year 2010",
...
# df_raw = pd.read_csv("1:1-4:30:21.csv")
# read input files
path = os.getcwd()
...
df_new = df_raw[df_raw['DATE'] == '2011-01']
df_new.reset_index(inplace=True)
x_cal_long = df_new['LONGITUDE'].tolist()
y_cal_lat = df_new['LATITUDE'].tolist()
z_cal = [0] * len(x_cal_long)
z_cal_temp = [item for item in (df_new['TAVG'].tolist())]


temp_mean = round(
    np.mean([item for item in z_cal_temp if np.isnan(item) == False]), 1)
...
def update_graph(option_slctd, slider_year):

    container = "The selected month is: {}".format(option_slctd)

    z_cal_temp = getTemperature(option_slctd)
    # print("x_cal_long", len(x_cal_long))
    # print("y_cal_lat", len(y_cal_lat))
    # print("z_cal", len(z_cal))
    # print("z_cal_temp", len(z_cal_temp))
    print("tri_cal.simplices", len(tri_cal.simplices))

    if len(z_cal_temp) < len(x_cal_long):
        for i in range(len(z_cal_temp), len(x_cal_long)):
            z_cal_temp.append(0)

    elif len(z_cal_temp) > len(x_cal_long):
        z_cal_temp = z_cal_temp[0:len(z_cal)]

    data2 = plotly_trisurf_cal(x_cal_long, y_cal_lat, z_cal, z_cal_temp,
                               tri_cal.simplices, colormap=cm.cubehelix, plot_edges=True)

    fig = go.Figure(data=data2, layout=layout)

    return container, fig, "You have selected: {} ".format(slider_year)
